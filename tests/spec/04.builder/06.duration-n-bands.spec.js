...
describe("scene duration", function() {
    ...
    var DEFAULT_SCENE_DURATION = anm.Scene.DEFAULT_LEN;
                      ...
                      { description: "when builder has some elements with no-defined bands",
                       ...
                       { description: "when builder has wide-band element",
                        ...
                        varyAll([ { description: "duration is 0", prepare: function() { _durationValue = 0; } },
                                  ...
                                  { description: "duration is negative", prepare: function() { _durationValue = -17.2; } } ], function() {

                            varyAll([ { description: "but duration is not set in any way",
                                        prepare: function() { _player.load(_builder); } },
                                      ...
                                      { description: "duration was set with the help of a property, but after loading",
                                        prepare: function() { _durationWasSet = false;
                                                              _player.load(_builder);
                                                              _builder.duration = _durationValue; } } ], function() {

                                it("duration should be overriden, if it was defined", function() {
                                    if (_durationWasSet) {
                                        expect(_durationValue).not.toEqual(DEFAULT_SCENE_DURATION);
                                ...
                                it("duration should be default, or infinite for dynamic mode, if it was not especially set", function() {
                                    if (!_durationWasSet) {
                                        ...
                                        expect(_builder.v.scene.duration).toBe(notDynamic ? _durationValue : Infinity);
                                        expect(_player.anim.duration).toBe(notDynamic ? _durationValue : Infinity);
                                        expect(_player.state.duration).toBe(notDynamic ? _durationValue : Infinity);
                                    }
                                });
                                ...
                                it("duration property of builder should be set, if duration iself was defined", function() {
                                    if (_durationWasSet) {
                                        expect(_builder.duration).toBeDefined();
                                        expect(_builder.duration).toBe(_durationValue > 0 ? _durationValue : 0);
                                    } else {
                                        expect(_builder.duration).not.toBeDefined();
                                    }
                                });
